...
- Modes: non-interactive; defaults follow config; `--full-auto` is an alias for `-a on-request` + `--sandbox workspace-write`; `--sandbox` controls file access (read-only, workspace-write, danger-full-access).
